...
describe('Spartacus Schematics: ng-add', () => {
    ...
    it('should add spartacus with PWA via passed parameter', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (buffer) {
            const webmanifestJSON = JSON.parse(buffer.toString(constants_1.UTF_8));
    ...
    it('should add spartacus with SSR via passed parameter', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (packageJsonBuffer) {
            const packageJSON = JSON.parse(packageJsonBuffer.toString(constants_1.UTF_8));
        ...
        if (appServerModuleBuffer) {
            const appServerModuleContent = appServerModuleBuffer.toString(constants_1.UTF_8);
            expect(appServerModuleContent.includes('ServerTransferStateModule')).toBeTruthy();
        }
    }));
    describe('@angular/localize', () => {
        it('should provide import in polyfills.ts and main.server.ts if SSR enabled', () => __awaiter(void 0, void 0, void 0, function* () {
            const tree = yield schematicRunner
                .runSchematicAsync('ng-add', Object.assign(Object.assign({}, defaultOptions), { ssr: true }), appTree)
                .toPromise();
            const polyfillsPath = file_utils_1.getPathResultsForFile(appTree, 'polyfills.ts', '/src')[0];
            const buffer = tree.read('./server.ts');
            const polyfillsBuffer = tree.read(polyfillsPath);
            expect(buffer).toBeTruthy();
            expect(polyfillsBuffer).toBeTruthy();
            if (buffer) {
                const appServerTsFileString = buffer.toString(constants_1.UTF_8);
                expect(appServerTsFileString.includes("import '@angular/localize/init'")).toBeTruthy();
            }
            if (polyfillsBuffer) {
                const polyfills = polyfillsBuffer.toString(constants_1.UTF_8);
                expect(polyfills.includes("import '@angular/localize/init'")).toBeTruthy();
            }
        }));
    });
});
